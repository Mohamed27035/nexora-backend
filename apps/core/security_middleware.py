...
APP_NAME = os.getenv("RSS_SOC_APP_NAME", "nexora-api").strip() or "nexora-api"
SOC_URL = os.getenv("RSS_SOC_URL", "http://198.199.70.48:3100").strip()
SOC_TOKEN = os.getenv("RSS_SOC_TOKEN", "rssbank-token-2024").strip()
ENV_NAME = os.getenv("RSS_SOC_ENV", "production").strip() or "production"
LOCAL_LOGS = os.getenv("RSS_SOC_LOCAL_LOGS", "/app/logs").strip() or "/app/logs"
BLOCK_ATTACKS = os.getenv("RSS_SOC_BLOCK_ATTACKS", "true").strip().lower() == "true"
BAN_DURATION = int(os.getenv("RSS_SOC_BAN_DURATION", "3600").strip() or "3600")

SEND_EMAIL = os.getenv("RSS_SOC_EMAIL_ALERTS", "false").strip().lower() == "true"
ALERT_EMAIL = os.getenv("RSS_SOC_ALERT_EMAIL", "").strip()
SMTP_HOST = os.getenv("RSS_SOC_SMTP_HOST", "smtp.gmail.com").strip()
SMTP_PORT = int(os.getenv("RSS_SOC_SMTP_PORT", "587").strip() or "587")
...
def _block_403(threat):
    ...
    return HttpResponseForbidden(body, content_type="application/json")
...
def _block_429(ip):
    ...
    response["Retry-After"] = str(BAN_DURATION)
    return response
...
def _send_email_alert(subject, body):
    ...
    def _send():
        try:
            ...
            print(f"[rss-soc] email failed: {error}")

    threading.Thread(target=_send, daemon=True).start()


def _create_bruteforce_table(cursor):
        ...
        try:
            with transaction.atomic():
                with connection.cursor() as cursor:
                    _create_bruteforce_table(cursor)
            _table_ready = True
        except Exception as error:
            print(f"[rss-soc] brute-force table init failed: {error}")
...
def _record_login_failure(ip):
    ...
    try:
            ...
            with connection.cursor() as cursor:
                ...
                if vendor == "postgresql":
                    ...
                    cursor.execute(
                        """
                        SELECT COUNT(*) FROM rss_soc_login_failures
                        WHERE ip = %s AND ts > datetime('now', '-5 minutes')
                        """,
                        [ip],
                    )
        ...
        print(f"[rss-soc] brute-force DB error: {error}")
        return 0
...
def _loki_worker():
    ...
    while True:
        ...
        for event in batch:
            ...
            streams_by_labels.setdefault(key, []).append(event)
            ...
            streams.append(
                {
                    "stream": dict(key),
                    "values": [
                        [str(int(event["ts_unix"] * 1e9)), json.dumps(event["data"])]
                        for event in events
                    ],
                }
            )
...
def _push_event(event_type, data, job, alert=False):
    ...
    try:
        ...
        file_name = "security.log" if job == "django-security" else "access.log"
        with open(os.path.join(LOCAL_LOGS, file_name), "a", encoding="utf-8") as handle:
